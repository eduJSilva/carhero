...
export class MaterialService {
  ...
  private materialesUrl = 'api/materiales';  // URL to web api
...
getMateriales(): Observable<Material[]> {
  return this.http.get<Material[]>(this.materialesUrl)
  .pipe(
    tap(_ => this.log('fetched materiales')),
    catchError(this.handleError<Material[]>('getMateriales', []))
  );
}
...
getMaterial(id: number): Observable<Material> {
  const url = `${this.materialesUrl}/${id}`;
  ...
  searchMateriales(term: string): Observable<Material[]> {
    ...
    return this.http.get<Material[]>(`${this.materialesUrl}/?nombre=${term}`).pipe(
      tap(x => x.length ?
...
updateMaterial(material: Material): Observable<any> {
  return this.http.put(this.materialesUrl, material, this.httpOptions).pipe(
...
addMaterial(material: Material): Observable<Material> {
  return this.http.post<Material>(this.materialesUrl, material, this.httpOptions).pipe(
